...
void load_data(const string filename, vector<vector<Cell>>& cells, int& start_y, int& start_x)
{
	...
	while (getline(fs, line)) {
		...
		char data;
        Cell cell;
		for (x = 0, ss.str(line), v.clear(); ss >> data; v.push_back(cell), x++) {
            cell = get_action(data);
            //cout << x << ", " << y << " : " << data << endl; //// debug
            if (cell == Self) {
                start_x = x;
                start_y = y;
                cell = Empty;
...
int main(){
    ...
    string filename("sample02.desc");
    vector<vector<Cell>> field;
    load_data(filename, field, start_y, start_x);
